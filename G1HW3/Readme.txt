...
② 큐
- waiting_queue: 클라이언트로부터 수신한 수식을 처리하기 전에 대기
...
- failed_queue: 클라이언트로부터 누락된 순번의 수식에 대해 재전송을 요청받은 경우 이 큐에 순번을 추가하여 재전송 대기 상태로 만듦
...
④ 클라이언트와 통신
- waiting(): 클라이언트로부터 데이터를 수신하고, 특정 형식(EXIT 및 SEND)의 메시지를 처리. 순번이 누락된 경우 클라이언트에 FAILED 메시지를 보내 재전송을 요청
...
◆ client.py 구성요소
① 큐
- failed_queue: 서버에서 특정 순번의 데이터 누락으로 인해 재전송 요청을 받은 경우 해당 순번을 여기에 저장하여 재전송을 준비

② 서버에 수식 전송
- send_expressions(): 로컬 파일에서 수식을 읽어 순차적으로 서버로 전송합니다. FAILED 메시지를 수신하여 재전송 요청을 받으면 해당 순번의 수식을 재전송

③ 서버로부터 결과 수신
- receive_results(): 서버로부터 수식 계산 결과를 수신하고, 순번이 누락된 경우 이를 확인하여 재전송 큐에 추가. 설정된 최대 수신 횟수에 도달하면 서버에 EXIT 메시지를 보내 연결 종료를 요청
